...
窗口控件也是一种容器。窗口只能容纳一个控件。

Q：那么如何向窗口中添加多个控件呢？

A：可以向窗口中加一个能容纳多个控件 的容器，再向此容器中添加其他的控件。



GTK+2.0中能容纳多个控件的容器主要有盒状容器（GtkBox）
表格容器（GtkTable）
按钮盒（GtkButtonBox）
分隔面版(GtkPanel)
固定布局（GtkFixed）
...
盒状容器是一种按一定顺序和方向紧密排列多个控件的容器。
在盒状容器(GtkBox)基
础上又分成纵向盒状容器(GtkVBox)和横向盒状容器(GtkHBox),两者除了创建和排列控件
的方式不一样外,其使用方式是一模一样的。


应注意的是容器中还能容纳其他容器,这形成了用 GTK+2.0 创建复杂用户界面
的基础。一定要把容器间容纳与被容纳关系理顺。--》数学问题。
...
另一个常用的与容器相关的函数是：
gtk_container_set_border_width  设定容器边框的宽度 不设定窗口边框的宽度,其默认值是 0
宽度的单位是像素
